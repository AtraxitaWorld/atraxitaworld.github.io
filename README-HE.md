...
assets/css/         סגנונות בסיס ואנימציות (הגופנים נטענים מ־Google Fonts דרך <link> ב־<head>)
...
* טיקטוק ואינסטגרם אינם מחוברים ולעולם לא מוצג עליהם תג שידור.
...
* שדה `url` מתקבל רק אם הפרוטוקול הוא HTTPS והמארח הוא `kick.com` (לקיק) או `youtube.com` / `youtu.be` (ליוטיוב), כולל תת־דומיינים שלהם. קידומת `www.` מוסרת לפני הבדיקה. בכל מקרה אחר נעשה שימוש בקישור הקבוע של הכרטיס.
...
* שני אייקוני השרתים המונפשים הומרו ל־WebP מונפש ב־160x160 (`srv-community.webp`, `srv-dm.webp`) — האנימציה נשמרה והמשקל ירד מ־13.1MB לכ־0.9MB.
* ששת אייקוני השרתים נטענים רק בפעם הראשונה שפאנל הדיסקורד נפתח (`data-src` שמוחלף ל־`src`), ולכן הם אינם נטענים כלל בטעינת העמוד.
* החלפת הלוגו: החליפו את `assets/images/atraxita-logo-clean.png` ואת הקבצים ב־`assets/icons/` באותם שמות.
* תמונת השיתוף לרשתות היא `assets/atraxita-social-og-1200x630.png` והיא כבר מוגדרת ב־`og:image` ככתובת מלאה. בהחלפתה יש לשמור על אותו שם ועל היחס 1200x630, ולעדכן את פרמטר ה־`?v=` כדי לרענן את מטמון הקישורים ברשתות.


## רקע וידאו

רקע האתר הוא סרטון לופ קצר: `assets/video/bg-loop.mp4` (2.4MB, ללא סאונד). הסרטון מוגדר `preload="none"` ונטען רק במסכים ברוחב 620px ומעלה, ולכן אינו יורד כלל בטלפונים.
